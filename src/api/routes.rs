...
pub fn create_router<S: Store + 'static>() -> Router<Arc<S>> {
    Router::new()
        ...
        // Database-level queries (automatically use main branch)
        .route(
        ...
        // Schema management (one per branch)
        .route(
        ...
        // Instance management (many per branch)
        .route(
        ...
        // Working Commit Staging Routes - these intercept changes when there's an active working commit
        .route(
        ...
        .route(
            ...
            patch(handlers::stage_instance_property_update::<S>),
        )
}
