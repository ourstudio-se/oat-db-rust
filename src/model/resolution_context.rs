...
impl ResolutionContext {
    ...
    pub fn with_description(mut self, description: String) -> Self {
        let metadata = self
            .metadata
            .get_or_insert_with(|| ResolutionContextMetadata {
                description: None,
                tags: Vec::new(),
...
mod tests {
    ...
    fn test_strict_resolution_context() {
        ...
        assert_eq!(
            context.policies.cross_branch_policy,
            CrossBranchPolicy::Reject
        );
        assert_eq!(
            context.policies.missing_instance_policy,
            MissingInstancePolicy::Fail
        );
        assert_eq!(
            context.policies.empty_selection_policy,
            EmptySelectionPolicy::Fail
        );
    ...
    fn test_permissive_resolution_context() {
        ...
        assert_eq!(
            context.policies.cross_branch_policy,
            CrossBranchPolicy::Allow
        );
        assert_eq!(
            context.policies.missing_instance_policy,
            MissingInstancePolicy::Skip
        );
        assert_eq!(
            context.policies.empty_selection_policy,
            EmptySelectionPolicy::Allow
        );
